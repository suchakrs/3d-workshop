...
// Parent group (used when "parentGroup" toggle is on)
const group = new THREE.Group();
group.add(mesh);
// Start at origin so the cube matches the position sliders (0,0,0) on load.
// The orbit offset (2,0,0) is applied only when the parent group is enabled
// (see the toggle below).
mesh.position.set(0, 0, 0);

// Start mesh at origin, not nested; nesting happens via toggle below
scene.add(mesh);
...
groupFolder
  ...
  .onChange((val: boolean) => {
    if (val) {
      // Nest mesh under rotating group; preserve world position
      scene.remove(mesh);
      mesh.position.set(2, 0, 0); // non-zero offset so child orbits
      group.rotation.set(0, 0, 0);
      scene.add(group);
    } else {
      // Detach: put mesh back in scene at origin
      scene.remove(group);
...
function animate() {
  ...
  // TODO (E1): when the parent group is enabled, rotate it so the child orbits the parent.
  // Hint: group.rotation.y += dt * groupSpeed;  (and the child already has a non-zero offset of x=2)
